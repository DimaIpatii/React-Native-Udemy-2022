...
const Navigation = (): JSX.Element => {
  const [selectedScreen, setSelectedScreen] = useState<
    Direction.AllExpences | Direction.RecentExpences | null
  >(Direction.RecentExpences);
  ...
  const navigationHandler = (direction: Direction): void => {
    switch (direction) {
      case Direction.AllExpences:
        ...
        navigate(
          Direction.AllExpences as never,
          { name: Direction.AllExpences } as never
        );
        break;
      case Direction.RecentExpences:
        setSelectedScreen(Direction.RecentExpences);
        navigate(
          Direction.RecentExpences as never,
          { name: Direction.RecentExpences } as never
        );
        break;
      case Direction.ManageExpence:
        navigate(
          Direction.ManageExpence as never,
          { name: Direction.ManageExpence } as never
        );
        break;
    }
  };
        ...
        <IconButton
          Icon={
            <EvilIcons
              ...
              size={30}
              color={
                selectedScreen === Direction.RecentExpences
                  ? colors.primary600
                  : colors.primary500
              }
            />
        ...
        <IconButton
          Icon={
            <EvilIcons
              ...
              size={30}
              color={
                selectedScreen === Direction.AllExpences
                  ? colors.primary600
                  : colors.primary500
              }
            />
